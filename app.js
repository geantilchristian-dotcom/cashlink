...
/* ── HELPERS DB (même interface qu'avant) ─────────────────────────────────── */
const dbFindOne = (colName, q) => col(colName).findOne(q);

const dbFind = (colName, q, sort) => {
    let cursor = col(colName).find(q);
    if (sort) cursor = cursor.sort(sort);
    return cursor.toArray();
};

const dbInsert = async (colName, doc) => {
    const d = { ...doc };
    if (!d._id) d._id = genId(colName.charAt(0).toUpperCase());
    await col(colName).insertOne(d);
    return d;
};

const dbUpdate = (colName, q, update, opts = {}) => {
    if (opts.multi) return col(colName).updateMany(q, update);
    return col(colName).updateOne(q, update);
};

const dbRemove = (colName, q, opts = {}) => {
    if (opts.multi) return col(colName).deleteMany(q);
    return col(colName).deleteOne(q);
};
...
app.post('/register', async (req, res) => {
    try {
        ...
        const doc = {
            id: uid, username: username.trim(), phone: String(phone).trim(),
...
/* CRON : gains quotidiens (toutes les 30 min) */
setInterval(async () => {
    try {
...
async function start() {
    try {
        ...
        console.log('[DB] MongoDB Atlas connecté avec succès !');
