/**
   * CASHLINK v20.0 - STOCKAGE /tmp GARANTI + TOUTES ROUTES
   */
  ...
  app.use(session({
      secret: 'cashlink_titan_2026',
      resave: false,
      saveUninitialized: false,
      cookie: { maxAge: 7 * 24 * 60 * 60 * 1000 }
  }));

  /* ── BASE DE DONNÉES (utilise /tmp - toujours accessible en écriture) ─── */
  const DB_DIR = process.env.DB_PATH || '/tmp/cashlink-data';
  try { fs.mkdirSync(DB_DIR, { recursive: true }); } catch(e) {}
  console.log('[DB] Dossier:', DB_DIR);

  const usersDB = new Datastore({ filename: path.join(DB_DIR, 'users.db'),  autoload: true });
  const txDB    = new Datastore({ filename: path.join(DB_DIR, 'tx.db'),     autoload: true });
  const retDB   = new Datastore({ filename: path.join(DB_DIR, 'ret.db'),    autoload: true });
  const logDB   = new Datastore({ filename: path.join(DB_DIR, 'logs.db'),   autoload: true });

  usersDB.ensureIndex({ fieldName: 'username', unique: true });
  usersDB.ensureIndex({ fieldName: 'phone',    unique: true });

  const dbFindOne = (db, q)    => new Promise((ok,ko) => db.findOne(q, (e,d) => e?ko(e):ok(d)));
  const dbFind    = (db, q, s) => new Promise((ok,ko) => { let cur = db.find(q); if(s) cur=cur.sort(s); cur.exec((e,d) => e?ko(e):ok(d)); });
  const dbInsert  = (db, d)    => new Promise((ok,ko) => db.insert(d, (e,r) => e?ko(e):ok(r)));
  const dbUpdate  = (db, q, u, o={}) => new Promise((ok,ko) => db.update(q,u,o,(e,n)=>e?ko(e):ok(n)));
  const dbRemove  = (db, q, o={})    => new Promise((ok,ko) => db.remove(q,o,(e,n)=>e?ko(e):ok(n)));
  ...
  const addLog = async t => { try { await dbInsert(logDB,{text:t,date:new Date().toLocaleString('fr-FR')}); }catch(e){} };
  ...
  app.post('/register', async (req,res) => {
      try {
          ...
          const doc = { id:'U'+Date.now(), username:username.trim(), phone:String(phone).trim(),
          ...
          const saved = await dbInsert(usersDB, doc);
          await addLog('Nouveau membre : '+saved.username);
          req.session.userId = saved.id;
          req.session.save(() => res.redirect('/dashboard'));
      } catch(e) {
          if (e.errorType==='uniqueViolated') return res.send(alertBack('Ce nom ou numéro est déjà utilisé.'));
  ...
  app.post('/recovery', async (req,res) => {
      try {
          ...
          const u = await dbFindOne(usersDB, {phone:String(phone).trim(), q_naissance, q_pere});
  ...
  app.get('/depot', authUser, async (req,res) => {
      try { const user=await dbFindOne(usersDB,{id:req.session.userId}); res.render('depot',{user,content:cfg}); }
  ...
  app.post('/process-depot', authUser, async (req,res) => {
      try {
          ...
          await dbInsert(txDB,{id:'TX'+Date.now(),utilisateur:user.username,username:user.username,
  ...
  app.post('/activer-gain', authUser, async (req,res) => {
      try {
          ...
          if (user.pack!=='Aucun' && progress(user)>=100) {
              ...
              await dbUpdate(usersDB,{id:user.id},{$inc:{bonus:gain},$set:{investDate:new Date()}});
          }
  ...
  app.get('/retrait', authUser, async (req,res) => {
      try {
          const user=await dbFindOne(usersDB,{id:req.session.userId});
          const transactions=await dbFind(retDB,{userId:user.id},{createdAt:-1});
  ...
  app.post('/retrait', authUser, async (req,res) => {
      try {
          ...
          await dbInsert(retDB,{id:'RET'+Date.now(),username:user.username,phone:user.phone,
  ...
  app.get('/admin', authAdmin, async (req,res) => {
      try {
          ...
          const pending    = await dbFind(txDB,{statut:'EN_ATTENTE'},{createdAt:-1});
          const pendingRet = await dbFind(retDB,{statut:'En attente'},{createdAt:-1});
          const approved   = await dbFind(txDB,{statut:'APPROUVE'});
          const logs       = await dbFind(logDB,{},{_id:-1});
          ...
          res.render('admin',{stats,transactions,retraits:pendingRet,users:allUsers,
              site:cfg,content:cfg,packs:PACKS,logs:logs.slice(0,50)});
      } catch(e) { res.status(500).send('Erreur admin: '+e.message); }
  });

  app.post('/valider-depot', authAdmin, async (req,res) => {
      try {
          const tx=await dbFindOne(txDB,{_id:req.body.txId});
          if (!tx) return res.redirect('/admin');
          const bonuses={BRONZE:5000,SILVER:9000,GOLD:40000,DIAMOND:120000};
          await dbUpdate(usersDB,{id:tx.userId},{$set:{pack:tx.pack,investDate:new Date()},$inc:{solde:tx.montant,bonus:bonuses[tx.pack]||0}});
          const user=await dbFindOne(usersDB,{id:tx.userId});
          if (user&&user.referredBy) { await dbUpdate(usersDB,{id:user.referredBy},{$inc:{bonus:Math.floor(tx.montant*0.10)}}); }
          await dbUpdate(txDB,{_id:tx._id},{$set:{statut:'APPROUVE'}});
  ...
  app.post('/admin/reject-tx', authAdmin, async (req,res) => {
      try { await dbUpdate(txDB,{_id:req.body.txId},{$set:{statut:'REJETE'}}); res.redirect('/admin'); }
      catch(e) { res.redirect('/admin'); }
  });

  app.post('/admin/approve-retrait', authAdmin, async (req,res) => {
      try { await dbUpdate(retDB,{_id:req.body.retId},{$set:{statut:'Payé'}}); res.redirect('/admin'); }
      catch(e) { res.redirect('/admin'); }
  });

  app.delete('/admin/delete-user/:id', authAdmin, async (req,res) => {
      try { await dbRemove(usersDB,{_id:req.params.id}); res.json({ok:true}); }
  ...
  setInterval(async ()=>{
      try {
          const active=await dbFind(usersDB,{pack:{$ne:'Aucun'}});
          for(const u of active){
              if(!u.investDate) continue;
              if(Date.now()-new Date(u.investDate).getTime()>=86400000){
                  await dbUpdate(usersDB,{id:u.id},{$inc:{bonus:cfg['daily_'+u.pack]||0},$set:{investDate:new Date()}});
              }
          }
  ...
  app.listen(PORT,()=>console.log('[CASHLINK v20.0] Port '+PORT+' | DB: '+DB_DIR));
